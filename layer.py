...
class Attention(nn.HybridBlock):
    ...
    def ScaledDotProductAttention(self, F, q, k, v):
    ...
    def hybrid_forward(self, F, q, k, v, Wq, Wk, Wv):
        return self.ScaledDotProductAttention(F, F.dot(q, Wq), F.dot(k, Wk), F.dot(v, Wv))
...
class SANet(gluon.HybridBlock):
    def __init__(self, shape, Vocab_len, h, is_PE=True, **kwargs):
        super(SANet, self).__init__(**kwargs)
        self.embed = nn.Embedding(input_dim=Vocab_len, output_dim=shape[0])
...
class LSTMANet(gluon.HybridBlock):
    def __init__(self, shape, Vocab_len, h,  num_hiddens, embed_size, num_layers, is_PE, **kwargs):
        super(LSTMANet, self).__init__(**kwargs)
        self.embed = nn.Embedding(input_dim=Vocab_len, output_dim=shape[0])
        self.PE = Position_Embedding(shape=(shape[1], shape[0]))
        self.MHA = Multi_Head_Attention(
            Q_shape=shape, K_shape=shape, V_shape=shape, h=h)
        self.liner = gluon.nn.Dense(2)
        self.pool = gluon.nn.GlobalAvgPool1D()
        self.drop = gluon.nn.Dropout(.5)
        self.shape = (shape[1], shape[0])
        self.Is_PE = is_PE
        self.lstm = rnn.LSTM(num_hiddens, num_layers=num_layers,
