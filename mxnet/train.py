...
def readIMDB(dir_url, seg='train'):
    ...
    for lb in pos_or_neg:
        ...
        for file in tqdm(files):
            with open(
                dir_url + '/' + seg + '/' + lb + '/' + file, 'r', encoding='utf8') as rf:
...
net = SANet(shape=(embedding_size, max_len), Vocab_len=len(vocab), h=8, is_PE=True)
...
for epoch in range(num_epochs):
    ...
    print('[epoch %d] train loss %.6f, train accuracy %.6f' % (epoch, train_loss, train_acc))
    print('[epoch %d] test loss %.6f, test accuracy %.6f' % (epoch, test_loss, test_acc))

    print('[epoch %d] throughput %.2f samples/s' %
          (epoch, (batch_size * len(x_train)) / (time.time() - start_epoch_time)))
